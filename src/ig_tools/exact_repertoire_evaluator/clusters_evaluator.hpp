...
struct Metrics {
	...
	struct OriginalClustersMetrics {
		...
		void Print(ostream &out) {
			...
			out << "#original singletons\t\t\t\t" << num_singletons << endl;
			out << "max original cluster\t\t\t\t" << max_cluster << endl;
		}

		void PrintCSV(ostream &out) {
			out << "#original clusters\t" << num_clusters << endl;
			out << "#not merged\t" << num_not_merged << endl;
			out << "#not merged (not trivial + singletons)\t" << num_nm_big_singletons << endl;
			out << "#original singletons\t" << num_singletons << endl;
	...
	struct ConstructedClustersMetrics {
		...
		void Print(ostream &out) {
			...
			out << "#constructed singletons\t\t\t\t" << num_singletons << endl;
			out << "max constructed cluster\t\t\t\t" << max_cluster << endl;
		}
	...
	double min_perc_identity;
	double avg_perc_identity;
	...
	void Print(ostream &out) {
		...
		out << "min percentage of identity (%)\t\t\t" << min_perc_identity * 100 << endl;
		out << "avg percentage of identity (%)\t\t\t" << avg_perc_identity * 100 << endl;
		out << "max percentage of identity (%)\t\t\t" << max_perc_identity * 100 << endl;
	}

	void PrintCSV(ostream &out) {
		original_metrics.PrintCSV(out);
		constructed_metrics.PrintCSV(out);
        out.precision(3);
		out << "avg fill-in\t" << avg_fillin << endl;
		out << "max cluster fill-in\t" << max_cluster_fillin<< endl;
		out << "#correct singletons\t" << correct_singletons << endl;
...
class ClustersEvaluator {
	...
	string constructed_rcm_fname_;
	string constructed_fa_fname_;
	...
	vector <pair <double, size_t> > length_perc_identity_;
	...
	void ComputeSequenceIdentityMetrics() {
		double sum_perc_identity = 0;
		for(auto it = original_clusters_.clusters_begin(); it != original_clusters_.clusters_end(); it++) {
			map <size_t, int> shared_clusters;
			for(auto read_it = it->second.begin(); read_it != it->second.end(); ++read_it) {
				string read_name = original_clusters_.Reads()[*read_it];
				size_t constr_cluster_id = constructed_clusters_.GetCluster(read_name);
				shared_clusters[constr_cluster_id] += 1;
			}

			size_t max_shared_cluster = size_t(-1);
			int max_shared_reads_num = -1;
			for (auto shared_it = shared_clusters.begin(); shared_it != shared_clusters.end(); ++shared_it) {
				if (shared_it->second > max_shared_reads_num) {
					max_shared_cluster = shared_it->first;
					max_shared_reads_num = shared_it->second;
				}
			}

			string orig_cluster_seq = original_clusters_.GetClusterSequence(it->first);
			string constr_cluster_seq = constructed_clusters_.GetClusterSequence(max_shared_cluster);
			size_t seq_matches = get_string_matches(orig_cluster_seq, constr_cluster_seq, 
				min(orig_cluster_seq.size(), constr_cluster_seq.size()), min(orig_cluster_seq.size(), constr_cluster_seq.size()));
			double perc_identity = double(seq_matches) / orig_cluster_seq.size();
			length_perc_identity_.push_back(make_pair(perc_identity, constructed_clusters_.GetClusterSequence(it->first).size()));
			sum_perc_identity += perc_identity;
			metrics_.min_perc_identity = min(metrics_.min_perc_identity, perc_identity);
			metrics_.max_perc_identity = max(metrics_.max_perc_identity, perc_identity);
		}
		metrics_.avg_perc_identity = sum_perc_identity / original_clusters_.ClustersNumber();
	}
...
public:
	ClustersEvaluator(char *original_rcm_fname, char *original_fa_fname, 
					  char *constructed_rcm_fname, char *constructed_fa_fname) :
		original_clusters_(original_rcm_fname, original_fa_fname),
		constructed_clusters_(constructed_rcm_fname, constructed_fa_fname),
		constructed_rcm_fname_(constructed_rcm_fname),
		constructed_fa_fname_(constructed_fa_fname){
		cout << "Extraction of original clusters from " << original_fa_fname << endl;
		original_clusters_.ExtractFromFile();
		cout << "Extraction of constructed clusters from " << constructed_fa_fname << endl;
	...
	void Evaluate() {
		...
		ComputeSequenceIdentityMetrics();
	}

	Metrics GetMetrics() { return metrics_; }

	Clusters& OriginalClusters() { return original_clusters_; }

	Clusters& ConstructedClusters() { return constructed_clusters_; }

	void PrintSizeIdentityPercentage(ostream &out) {
		for (auto length_perc_it = length_perc_identity_.begin(); length_perc_it != length_perc_identity_.end(); ++length_perc_it) {
			out << length_perc_it->first * 100 << '\t' << length_perc_it->second << '\n';
		}
	}
};

class ClusterSizeHistogramFiller {
	Clusters &clusters_;
	BaseHistogram<size_t> hist_;
public:
	ClusterSizeHistogramFiller(Clusters &clusters) :
		clusters_(clusters) {
		for(auto it = clusters_.clusters_begin(); it != clusters_.clusters_end(); it++)
			hist_.Add(it->second.size());
	}

	void WriteToFile(string filename) const {
		hist_.SimpleSavingToFile(filename);
	}
};
